...
\subsection{Bizagi}
\renewcommand{\progname}{\swname{Bizagi Process Manager}\xspace}
\begin{picture}(0,0)
...
\subsubsection{Descrizione}
\progname è un \sw BPM \inglese{open source} sviluppato dalla società inglese \swname{Bizagi}.
Il \sw si compone di due moduli:
\begin{itemize}
...
\midrule
modellazione                                 & \tick  & \tick  &      &      \\
modellazione tramite editor grafico          & \tick  & \tick  &      &      \\
rispetto notazione BPMN                      & \tick  & \cross &      &      \\
monitoraggio dei processi                    & \tick  & \tick  &      &      \\
sistema di \inglese{reporting}               & \tick  & ¿?      &      &      \\
indicatori grafici stato di avanzamento      & \cross & \tick  &      &      \\
ottimizzazione dei processi                  & \tick$^{*}$ & \cross &      &      \\
simulazione                                  & \tick  & \cross &      &      \\
sistema di \inglese{reporting} simulazione   & \tick  & \cross &      &      \\
validazione modelli                          & \tick  & \cross &      &      \\
esecuzione automatizzata processi            & \tick  & \tick  &      &      \\
integrazione con \sw di parti terze          & \tick  & \tick  &      &      \\
...
gestione separata dei ruoli                  & \tick$^{*}$  & ¿?  &      &      \\
\bottomrule
