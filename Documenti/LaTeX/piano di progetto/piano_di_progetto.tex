...
\begin{figure}[H]
  \centering
  \includegraphics[width=.75\textwidth]{bonita_architecture}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{simulazione}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{utente}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{amministratore}
...
sviluppo guidato di connettori                     & \tick  & \tick  & \tick  & \tick \\
...
connettore SAP con \inglese{wizard}                & \cross & \cross & \tick  & \tick \\
...
gestione diritti                                   & \cross & \tick  & \tick  & \tick \\
...
\begin{figure}[H]
  \centering
  \includegraphics[width=.8\textwidth]{pm_editor}
...
Infine, si segnala che la localizzazione in lingua italiana non è offerta nativamente ma solo come modulo integrativo e che il file PO (\inglese{Portable Object}) messo a disposizione dalla comunità di utilizzatori di \progname contiene tutte le stringhe per la traduzione dell'interfaccia, con il conseguente risultato che alcune voci di menu non contengono alcuna etichetta testuale, in quanto non viene visualizzata nemmeno la stringa originale.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=.8\textwidth]{merdometro}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=.8\textwidth]{pm_userlist}
  \caption{Interfaccia di amministrazione dei profili utente e gruppi.}
  \label{fig:pmuserlist}
\end{figure}

Infine, all'accesso al sistema, gli utenti cui sono state assegnate le attività possono visualizzare l'elenco dei rispettivi compiti all'interno del \inglese{case} corrente, vale a dire dell'istanza del processo aziendale attualmente in esecuzione, tramite una schermata simile a quella riportata in \figurename~\ref{fig:pmtasklist}.

\begin{figure}[H]
  \centering
  \includegraphics[width=.8\textwidth]{pm_dashboard}
...
\begin{picture}(0,0)
  \put(250, 10){\includegraphics[width=0.2\textwidth]{bizagi_logo}}
\end{picture}
...
\item \swname{Bizagi Suite}, \sw per la gestione ed il monitoraggio dei processi aziendali \end{itemize}

La soluzione offerta da \progname è completa ed è sicuramente alla portata di una PMI come \customer .
Si tratta infatti di un \sw particolarmente performante in grado di fornire informazioni molto precise sull'andamento dei processi.

\subsubsection{Requisiti di sistema}
\begin{itemize}
\item sistemi operativi \texttt{windows}
\item SQL Server Express 2008 SP3 e successivi
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{verifica}
  \caption{Messaggio restituito dalla funzionalità di verifica di \swname{Bizagi Process Modeler}}
  \label{fig:verifica}
\end{figure}
Per quanto riguarda invece, la gestione della lingua, l' italiano è offerto come funzionalità nativa del \sw \progname. Tuttavia, la localizzazione in lingua italiana non è completa e diverse parti risultano essere tradotte male o non tradotte.

 \progname permette la simulazione dei processi. In seguito ai test eseguiti, però, risulta essere più deludente rispetto a quella offerta da altri \sw precedentemente verificati.
I \inglese{report} restituiti dalla simulazione risultano essere poco comprensibili, incompleti e non contengono alcun grafico, che permette ai responsabili decisionali di realizzare molto più velocemente la situazione.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{suite}
  \caption{Interfaccia di \swname{Bizagi Suite}}
  \label{fig:suite}
\end{figure}


Come indicato tra i requisiti di sistema, \progname può essere installato solo su piattaforme \inglese{windows}. Questo, in generale, costituisce un limite, tuttavia, in questo caso, visto che \customer dispone di un'infrastruttura \inglese{windows-based}, non vi è alcun problema.

\swname{Bizagi Process Modeler} comprende una \inglese{future} che permette a più utenti di collaborare contemporaneamente alla modellazione di un processo. 
Si tratta di una funzionalità molto interessante, soprattutto in \inglese{team} numerosi, per parallelizzare il lavoro e quindi aumentare la produttività.

\subsection{Camunda BPM Platform}
\renewcommand{\progname}{\swname{Camunda}\xspace}
\begin{picture}(0,0)
  \put(250, 10){\includegraphics[width=.3\textwidth]{camunda_logo}}
\end{picture}

\subsubsection{Descrizione}
\progname è una soluzione realizzata da un \inglese{team} di sviluppatori dell'area tedesca e distribuita sotto licenza \inglese{Apache v2.0} e quindi totalmente \inglese{open source}. Il \sw si basa sulla piattaforma Java e nella sua distribuzione di base comprende tre \inglese{front-end} grafici indipendenti: \swname{Tasklist} utilizzato per la gestione dei flussi di lavoro basati sull'interazione con gli utenti umani, \swname{Cockpit} specializzato nella gestione e il controllo delle operazioni aziendali e \swname{Cycle} che permette di importare e rendere eseguibili i modelli di processi BPMN 2.0.
%TODO parlare del backend che ci sta dietro
...
\subsubsection{Caratteristiche}
\progname non comprende alcun editor visuale per la modellazione dei processi aziendali al suo interno: tale funzionalità è demandata a \sw di terze parti (ne esistono anche di \inglese{open source} come, ad esempio, \swname{Yaoqiang}) ma, essendo il BPMN un formato \inglese{standard} questo non rappresenta un problema in sede di importazione dei dati all'interno del sistema tramite \swname{Cicle}.
...
\midrule
modellazione                                 & \tick  & \tick  & \tick      &      \\
modellazione tramite editor grafico          & \tick  & \tick  & \tick     &      \\
rispetto notazione BPMN                      & \tick  & \cross &  \tick     &      \\
monitoraggio dei processi                    & \tick  & \tick  &  \tick    &      \\
sistema di \inglese{reporting}               & \tick  & ¿?      &  \tick    &      \\
indicatori grafici stato di avanzamento      & \cross & \tick  &   ¿?    &      \\
ottimizzazione dei processi                  & \tick$^{*}$ & \cross & ¿?       &      \\
simulazione                                  & \tick  & \cross &  \tick       &      \\
sistema di \inglese{reporting} simulazione   & \tick  & \cross &  \tick      &      \\
validazione modelli                          & \tick  & \cross &  \tick     &      \\
esecuzione automatizzata processi            & \tick  & \tick  &   ¿?  &        \\
integrazione con \sw di parti terze          & \tick  & \tick  &   \tick     &      \\
interfacciamento con utenti                  & \tick  & \tick  &    ¿?    &      \\
\inglese{form} statici                       & \tick  & \tick  &    ¿?    &      \\
\inglese{form} dinamici                      & \tick$^{*}$ & \tick  &¿?        &      \\
\inglese{form} internazionalizzabili         & \tick$^{*}$ & ¿?  &   ¿?     &      \\
gestione separata dei ruoli                  & \tick$^{*}$  & ¿?  & \cross       &      \\
\bottomrule
